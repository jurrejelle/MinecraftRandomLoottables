...
def edit_amount(contents):
        data = json.loads(contents)
        done = False
        try:
                old = data['pools'][0]['rolls']
                roll1 = random.randrange(0, 3)
                if(roll1<2):
                        new = random.randrange(1, 16)
                else:
                        new = random.randrange(16, 128)
                data['pools'][0]['rolls'] = new
                done = True
        except:
                pass#print('error')
        try:
                del data['pools'][0]['conditions']
        except:
                pass
        try:
                del data['pools']['entries'][0]['functions'][0]['conditions']
        except:
                pass
        if(done):
...
for file in file_list:
	...
	if(SWAP_1_FOR_1): del remaining[i]
	
zipbytes = io.BytesIO()
zip = zipfile.ZipFile(zipbytes, 'w', zipfile.ZIP_DEFLATED, False)

for from_file in file_dict:
	with open(file_dict[from_file]) as file:
		contents = file.read()
	if(RANDOMISE_AMOUNTS): contents = edit_amount(contents)
...
zip.writestr('data/{}/functions/reset.mcfunction'.format(datapack_name), 'tellraw @a ["",{"text":"Loot table randomizer by SethBling, modified by JurreJelle","color":"green"}]')
